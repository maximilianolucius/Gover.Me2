...
def extraer_datos_turismo_total():
    ...
    # Mapeo de archivos a categoría
    ARCHIVO_CATEGORIA = {
        '01_total_turistas_': 'total_andalucia',
        '02_espanoles_': 'total_espana',
        '05_extranjeros_': 'total_extranjeros',
        '06_total_': 'total_general'
    }
    ...
    try:
        # Buscar archivos de totales
        dir_in = './nexus/data/ultimos_datos_turisticos'
        archivos = []

        for prefijo in ARCHIVO_CATEGORIA.keys():
        ...
        for archivo_path, prefijo in archivos:
            ...
            # Determinar categoría y fecha
            categoria = ARCHIVO_CATEGORIA[prefijo]
            nombre = os.path.basename(archivo_path).lower()

            # Extraer fecha del nombre
            año, mes = 2025, 1  # Default
            match = re.search(r"(ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic)(\d{2})", nombre)
            if match:
                mes_map = {'ene': 1, 'feb': 2, 'mar': 3, 'abr': 4, 'may': 5, 'jun': 6,
                           'jul': 7, 'ago': 8, 'sep': 9, 'oct': 10, 'nov': 11, 'dic': 12}
                mes = mes_map.get(match.group(1), 1)
        ...
        for registro in datos_procesados:
            ...
            cursor.execute("""
                ...
                (año, mes, categoria, viajeros_hoteles, pernoctaciones_hoteles, 
        ...
        # Mostrar resultados
        cursor.execute("SELECT * FROM turismo_total ORDER BY año, mes, categoria")
        print("\nDatos en la tabla:")
        for row in cursor.fetchall():
            print(row)

        # Verificar datos específicos (test rápido)
        cursor.execute("""
            SELECT categoria, viajeros_hoteles, pernoctaciones_hoteles 
            FROM turismo_total 
            WHERE año = 2025 AND mes = 1 AND categoria = 'total_andalucia'
        """)

        result = cursor.fetchone()
        if result:
            cat, viajeros, pernoc = result
            print(f"\n✅ Test rápido: {cat} - Viajeros: {viajeros:,}, Pernoctaciones: {pernoc:,}")
            if viajeros and viajeros > 980000 and pernoc and pernoc > 2270000:
                print("✅ Valores coinciden con los esperados!")
            else:
                print("⚠️  Verificar valores extraídos")
