-- =============================================================================
-- RECREAR BASE DE DATOS NEXUS (solo para desarrollo)
-- =============================================================================
\set ON_ERROR_STOP on
\echo Reiniciando base de datos 'nexus' (con verificación de permisos)...

-- Cambiar a una base de mantenimiento para poder gestionar 'nexus'
\c postgres

-- Detectar si existe la BD y si el usuario actual puede borrarla
SELECT CASE WHEN EXISTS (SELECT 1 FROM pg_database WHERE datname = 'nexus') THEN '1' ELSE '' END AS db_exists \gset
SELECT CASE WHEN rolsuper THEN '1' ELSE '' END AS is_superuser FROM pg_roles WHERE rolname = current_user \gset
SELECT CASE WHEN (SELECT pg_get_userbyid(datdba) FROM pg_database WHERE datname = 'nexus') = current_user THEN '1' ELSE '' END AS is_owner \gset

\if :db_exists
  \if :is_superuser
    -- Finalizar conexiones y borrar/recrear
  ...
  \elif :is_owner
    ...
    \c nexus
  \else
    \echo No soy owner/superuser de 'nexus'. Limpiando esquema public dentro de la BD existente.
    \c nexus
  ...
  \endif
\else
  -- Si no existe, intentar crearla (requiere CREATEDB). Si falla, el script se detendrá aquí.
  CREATE DATABASE nexus;
  \c nexus
\endif
